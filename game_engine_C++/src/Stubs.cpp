...
class Renderer {
public:
    Renderer(SDL_Renderer* renderer) {} // Constructeur prenant un pointeur vers le renderer SDL (non utilisé ici)
};
...
class AssetManager {
public:
    AssetManager(SDL_Renderer* renderer) {} // Constructeur prenant un pointeur vers le renderer SDL (non utilisé ici)
};

// Classe stub pour la gestion de l'audio (musique, effets sonores)
class AudioManager {
public:
    void playSound(const std::string& name) {} // Méthode pour jouer un son en fonction de son nom (vide pour le stub)
};

// Classe stub pour la gestion du score du joueur
class ScoreManager {
public:
    void addScore(int score) {} // Méthode pour ajouter des points au score (vide pour le stub)
};

// Classe stub pour la gestion de l'authentification utilisateur
class AuthSystem {
public:
    bool login(const std::string& user, const std::string& pass) { return true; } // Méthode de connexion simulée (retourne toujours vrai)
};
